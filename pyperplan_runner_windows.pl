...
run_pyperplan_soln(Exe, Domain, Problem, Actions) :-
    ...
    ( catch(process_create(Exe, [Domain, Problem],
                           [ stdout(null),              % don't capture logs
                             stderr(pipe(Err)),         % capture errors
                             process(PID)
                           ]), 
             error(existence_error(source_sink, _), _),
             % If direct executable fails, try with python -m
             ( format('Direct execution failed, trying with python -m...~n', []),
               catch(process_create('python', ['-m', Exe, Domain, Problem],
                                    [ stdout(null),
                                      stderr(pipe(Err2)),
                                      process(PID2)
                                    ]), 
               ...
               ;  ( Status2 = exit(0)
                  ...
                  ;  format(user_error, "~s", [Stderr]),
                     throw(error(pyperplan_failed(Status2), _))
                  )
               )
             )
